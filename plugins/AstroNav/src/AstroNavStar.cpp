...
#include "AstroNavStar.hpp"
#include "AstroNavStars.hpp"
...
const QString AstroNavStar::ASTRONAVSTAR_TYPE=QStringLiteral("Navigation Star");
StelTextureSP AstroNavStar::markerTexture;
bool AstroNavStar::distributionMode = false;
bool AstroNavStar::timelineMode = false;
bool AstroNavStar::habitableMode = false;
bool AstroNavStar::showDesignations = false;
Vec3f AstroNavStar::exoplanetMarkerColor = Vec3f(0.4f,0.9f,0.5f);
Vec3f AstroNavStar::habitableAstroNavStarMarkerColor = Vec3f(1.f,0.5f,0.f);
int AstroNavStar::temperatureScaleID = 1;

AstroNavStar::AstroNavStar(const QVariantMap& map)
	: initialized(false)
	, EPCount(0)
	, PHEPCount(0)
	, designation("")
	...
	, sradius(0.)
	, effectiveTemp(0)
	, hasHabitableAstroNavStars(false)
{
	...
	hasHabitableAstroNavStars = map.value("hasHP", false).toBool();
...
AstroNavStar::~AstroNavStar()
{
	//
}

QVariantMap AstroNavStar::getMap(void) const
{
	QVariantMap map;
	...
	map["hasHP"] = hasHabitableAstroNavStars;
...
float AstroNavStar::getSelectPriority(const StelCore *core) const
{
	return StelObject::getSelectPriority(core)-2.f;
}

QString AstroNavStar::getNameI18n(void) const
{
...
QString AstroNavStar::getEnglishName(void) const
{
...
QString AstroNavStar::getDesignation(void) const
{
	return designation;
}

QStringList AstroNavStar::getAstroNavStarsEnglishNames() const
{
	return englishNames;
}

QStringList AstroNavStar::getAstroNavStarsNamesI18n() const
{
	return translatedNames;
}

QStringList AstroNavStar::getAstroNavStarsDesignations() const
{
	return exoplanetDesignations;
}

QString AstroNavStar::getInfoString(const StelCore* core, const InfoStringGroup& flags) const
{
	QString str;
	QTextStream oss(&str);
	const StelTranslator& trans = StelApp::getInstance().getLocaleMgr().getSkyTranslator();
	...
	if (flags&Extra)
	{
		...
		if (exoplanets.size() > 0)
		{
			...
			QString planetNameLabel = QString("<td style=\"%2\">%1</td>").arg(q_("AstroNavStar")).arg(qss);
...
QString AstroNavStar::getTemperatureScaleUnit() const
{
	QString um = "";
...
float AstroNavStar::getTemperature(float temperature) const
{
	float rt = 0.f;
	switch (temperatureScaleID) {
		case 0: // Kelvins
...
QVariantMap AstroNavStar::getInfoMap(const StelCore *core) const
{
	...
	map["sradius"] = sradius;
	map["effectiveTemp"] = effectiveTemp;
	map["hasHabitablePlanets"] = hasHabitableAstroNavStars;
	map["type"] = "AstroNavStarSystem"; // Replace default but confusing "AstroNavStar" from class name.
	// TODO: Maybe add number of habitables? Add details?
	return map;
}

QString AstroNavStar::getPlanetaryClassI18n(QString ptype) const
{
	QString result = "";
...
Vec3f AstroNavStar::getInfoColor(void) const
{
	return Vec3f(1.f, 1.f, 1.f);
}

float AstroNavStar::getVMagnitude(const StelCore* core) const
{
	Q_UNUSED(core);
	return (distributionMode ? 4.f : (Vmag<99. ? static_cast<float>(Vmag) : 6.f));
}

double AstroNavStar::getAngularSize(const StelCore*) const
{
	return 0.0001;
}

bool AstroNavStar::isDiscovered(const StelCore *core)
{
	int year, month, day;
	QList<int> discovery;
...
void AstroNavStar::update(double deltaTime)
{
	labelsFader.update(static_cast<int>(deltaTime*1000));
}

void AstroNavStar::draw(StelCore* core, StelPainter *painter)
{
	if ((habitableMode) &&  (!hasHabitableAstroNavStars)) {return;}
	...
	if (mag <= mlimit)
	{		
		AstroNavStar::markerTexture->bind();
		Vec3f color = (hasHabitableAstroNavStars ? habitableAstroNavStarMarkerColor : exoplanetMarkerColor);
